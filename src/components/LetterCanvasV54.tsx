...
import { renderTreeStrokes, trunkColorHex } from './treeMaterial';
...
export function LetterCanvasV54({
  letter,
  material,
  selectedSticker,
  strokes,
  ...
  function drawScene(nextStrokes: Stroke[]) {
    const canvas = canvasRef.current;
    if (!canvas || pixelWidth < 2 || pixelHeight < 2)
      return;

    const ordinaryStrokes = nextStrokes.filter(stroke => stroke.material !== 'tree');
    renderCanvas(canvas, maskRef.current, letter, ordinaryStrokes, stickers, null);
    renderTreeStrokes(canvas, maskRef.current, nextStrokes);
  }
  ...
  function setPreviewStyle(context: CanvasRenderingContext2D) {
    ...
    context.strokeStyle = material === 'water'
      ...
      : material === 'stone'
        ...
        : material === 'volcano'
          ? 'rgba(226,77,25,.9)'
          : trunkColorHex(treeOptions?.trunkColor);
    context.fillStyle = context.strokeStyle;
  }
